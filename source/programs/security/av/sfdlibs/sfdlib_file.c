/* sfdlib_file.c - This file is part of sfdlibs, the Userland C Libraries of the
...
int sfd_filesize(FILE*);

char *sfd_filetype(FILE*);
char *sfd_fileformatted(FILE*);
...
int sfd_filesize(FILE *f) {
    int size, pos = ftell(f);
    fseek(f, 0, SEEK_END);
    size = ftell(f);
    fseek(f, 0, pos);
    return size;
...
char *sfd_filesize_formatted(FILE *f) {
    double size = (double)sfd_filesize(f);
    char *s = malloc(13), *u;
    fseek(f, 0, SEEK_END);
    if (size >= 1073741824) {
        u = "GB";
